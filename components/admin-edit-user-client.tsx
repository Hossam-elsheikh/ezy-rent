...
export function AdminEditUserClient({ user, targetUser }: AdminEditUserClientProps) {
            ...
            <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
                ...
                <div className={`mb-8 ${isRTL ? 'text-right' : 'text-left'}`}>
                    <h1 className={`text-2xl font-bold text-gray-900 dark:text-white flex items-center gap-2 ${isRTL ? 'flex-row-reverse' : ''}`}>
                        <User size={22} className="text-indigo-500" /> {t.admin.edit_user}
                    </h1>
                    <p className="text-gray-500 dark:text-gray-400 mt-1 truncate">{targetUser.full_name} ({targetUser.email})</p>
                </div>

                <div className="bg-white dark:bg-gray-900 rounded-2xl p-6 shadow-sm border border-gray-100 dark:border-gray-800">
                    <AdminUserForm mode="edit" initialData={targetUser} />
                </div>
            </div>
        </div>
    );
